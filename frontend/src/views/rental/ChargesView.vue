<template>
  <section class="charges-screen">
    <PageHeader
      :title="tenantFilter.id ? 'Tenant charge statement' : 'Charges'"
    ...
    <div class="filter-bar print-hidden">
    ...
    <p v-if="message" class="alert-success mb-3 print-hidden">{{ message }}</p>
    <p v-if="generateError" class="alert-error mb-3 print-hidden">{{ generateError }}</p>

    <div id="charge-print-area">
      <div
        v-if="tenantFilter.id"
        class="print-only mb-4 hidden border-b border-zinc-300 pb-4 text-center"
      >
        <h1 class="text-xl font-semibold text-zinc-900">Tenant charge statement</h1>
        <p class="mt-1 text-sm text-zinc-700">{{ tenantFilter.name }}</p>
        <p class="text-xs text-zinc-500">Printed {{ printedAt }}</p>
      </div>
      ...
      >
        ...
        <template #cell-rent_amount="{ item }">
          {{ item.is_editable ? formatMoney(item.rent_amount) : '—' }}
        </template>
        <template #cell-service_amount="{ item }">
          {{ item.is_editable ? formatMoney(item.service_amount) : '—' }}
        </template>
...
import TenantChargeStatement from '../../components/rental/TenantChargeStatement.vue'
...
const pageSubtitle = computed(() => {
  if (tenantFilter.id) {
    return 'Monthly invoices grouped by billing period — rent, water, and electricity on separate lines.'
  }

  return 'Monthly rent, service, water, and electricity charges per tenant.'
})
...
async function load() {
  ...
  if (tenantFilter.id && !tenantFilter.name && charges.value.length > 0) {
    tenantFilter.name = charges.value[0].tenant_name || ''
  }
}
...
function clearTenantFilter() {
  ...
  tenantFilter.id = ''
  tenantFilter.name = ''
  router.replace({ name: 'rental.charges' })
  load()
}
