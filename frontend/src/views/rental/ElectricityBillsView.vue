<template>
  <section>
    <PageHeader
      ...
      :breadcrumbs="[{ label: 'Rental', to: '/rental' }, { label: 'Electricity' }]"
    >
      <template #actions>
    ...
    <DataTable
      ...
      money-module="rental"
      empty-message="No electricity readings found."
    ...
    <AppDialog
      ...
      :title="editing ? 'Edit electricity reading' : 'Record electricity reading'"
      size="md"
...
async function save() {
  ...
  try {
    ...
    if (editing.value) {
      await updateTenantElectricityBill(editing.value.id, payload)
      toast.success('Electricity reading updated.')
    } else {
      await createTenantElectricityBill(payload)
