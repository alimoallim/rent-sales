<template>
  <section>
    <PageHeader title="Water bills" subtitle="Tenant meter readings and water charges.">
      <template #actions>
    ...
    <ResponsiveDataList :items="bills" :columns="billColumns" empty-message="No water bills found.">
    ...
    <AppDialog v-model:open="showForm" title="Record water bill" size="md" :close-on-backdrop="false">
...
async function load() {
  ...
  const response = await fetchWaterBills(params)
...
async function save() {
  ...
  try {
    ...
    error.value = validation
      ...
      : e.response?.data?.message || 'Could not save water bill.'
  }
}

async function markPaid(bill) {
  try {
    await markWaterBillPaid(bill.id)
    await load()
  } catch (e) {
